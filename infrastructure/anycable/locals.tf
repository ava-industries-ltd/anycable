...
locals {
  ...
  anycable_container_environment_variables = [
    ...
    { "name" : "ANYCABLE_RPC_HOST", "value" : "${module.grpc.alb_dns_name}:${var.grpc_port}" },
  ...
  grpc_container_command = ["bundle", "exec", "anycable", "--rpc-host=0.0.0.0:${var.grpc_port}", "--require", "./config/environment.rb"]
